...
- [四、引用类型原子类](#四引用类型原子类)
  ...
  - [4.3 AtomicMarkableReference 类使用示例](#43-atomicmarkablereference-类使用示例)
- [五、对象的属性修改类型原子类](#五对象的属性修改类型原子类)
...
**CAS 的原理**是拿期望的值和原本的一个值作比较，如果相同则更新成新的值。UnSafe 类的 objectFieldOffset() 方法是一个本地方法，这个方法是用来拿到“原来的值”的内存地址。另外 value 是一个 volatile 变量，在内存中可见，因此 JVM 可以保证任何时刻任何线程总能拿到该变量的最新值。
...
基本类型原子类只能更新一个变量，如果**需要原子更新多个变量，需要使用 引用类型原子类。**
...
- `AtomicIntegerFieldUpdater`: 原子**更新整形字段**的更新器
- `AtomicLongFieldUpdater`: 原子**更新长整形字段**的更新器
- `AtomicReferenceFieldUpdater` : 原子**更新引用类型里的字段**的更新器

> **要想原子地更新对象的属性需要两步**

- 第一步，因为对象的属性修改类型原子类都是抽象类，所以每次使用都必须**使用静态方法 newUpdater()创建一个更新器**，并且需要**设置想要更新的类和属性。**
