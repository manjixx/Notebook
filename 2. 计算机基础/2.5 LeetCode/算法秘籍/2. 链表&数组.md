...
- 基本二分搜索:注意`while()`中循环条件与right的变换
- 左侧边界搜索:核心是`nums[mid] == target`时将右侧边界缩小，`[left,right)与[left,right]`均可使用，为统一推荐使用`[left,right]`返回时注意left越界判别`left >= nums.length || nums[left] != target`
- 右侧边界搜素:核心是`nums[mid] == target`时将右侧边界缩小，使用`[left,right]`，返回时注意right越界判别`right < 0 || nums[right] != target`,此时`[left,right)`存在问题，不推荐使用
- [详细内容见](#26-数组-二分查找左右指针)

#### 2.4.3.2 [两数之和](https://leetcode.cn/problems/two-sum-ii-input-array-is-sorted/description/)\

核心：因为数组是非递减有序数组，通过调整双指针`left`与`right`就可以调整 `sum` 的⼤⼩
