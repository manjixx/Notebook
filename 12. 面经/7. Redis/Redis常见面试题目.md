- [Redis 常见面试题汇总](#redis-常见面试题汇总)
  - [一、Redis 基础](#一redis-基础)
    - [1.1 什么是 Redis？](#11-什么是-redis)
    - [1.2 Redis 为什么这么快？](#12-redis-为什么这么快)
    - [1.3 分布式缓存常见的技术选型方案有哪些？](#13-分布式缓存常见的技术选型方案有哪些)
    - [1.4 说一下 Redis 和 Memcached 的区别和共同点](#14-说一下-redis-和-memcached-的区别和共同点)
    - [1.5 缓存数据的处理流程是怎样的？](#15-缓存数据的处理流程是怎样的)
    - [1.6 为什么要用 Redis/为什么要用缓存？](#16-为什么要用-redis为什么要用缓存)
    - [1.7 Redis 除了做缓存，还能做什么？](#17-redis-除了做缓存还能做什么)
    - [1.8 Redis 可以做消息队列么？](#18-redis-可以做消息队列么)
  - [二、Redis 数据结构](#二redis-数据结构)
    - [2.1 Redis 常用的数据结构有哪些？](#21-redis-常用的数据结构有哪些)
    - [2.2 String 的应用场景有哪些？](#22-string-的应用场景有哪些)
    - [2.3 String 还是 Hash 存储对象数据更好呢？](#23-string-还是-hash-存储对象数据更好呢)
    - [2.4 购物车信息用 String 还是 Hash 存储更好呢?](#24-购物车信息用-string-还是-hash-存储更好呢)
    - [2.5 使用 Redis 实现一个排行榜怎么做？](#25-使用-redis-实现一个排行榜怎么做)
    - [2.6 使用 Set 实现抽奖系统需要用到什么命令？](#26-使用-set-实现抽奖系统需要用到什么命令)
    - [2.7 使用 Bitmap 统计活跃用户怎么做？](#27-使用-bitmap-统计活跃用户怎么做)
    - [2.8 使用 HyperLogLog 统计页面 UV 怎么做？](#28-使用-hyperloglog-统计页面-uv-怎么做)
  - [三、Redis 线程模型](#三redis-线程模型)
    - [3.1 Redis 单线程模型了解吗？](#31-redis-单线程模型了解吗)
    - [3.2  Redis6.0 之前为什么不使用多线程？](#32--redis60-之前为什么不使用多线程)
    - [3.3 Redis6.0 之后为何引入了多线程？](#33-redis60-之后为何引入了多线程)
  - [四、Redis 内存管理](#四redis-内存管理)
    - [4.1 Redis 给缓存数据设置过期时间有啥用？](#41-redis-给缓存数据设置过期时间有啥用)
    - [4.2 Redis 是如何判断数据是否过期的呢？](#42-redis-是如何判断数据是否过期的呢)
    - [4.3 过期的数据的删除策略了解么？](#43-过期的数据的删除策略了解么)
    - [4.4 Redis 内存淘汰机制了解么？](#44-redis-内存淘汰机制了解么)
  - [五、Redis 持久化机制](#五redis-持久化机制)
    - [5.1 Redis如何实现数据不丢失？](#51-redis如何实现数据不丢失)
    - [5.2 什么是 RDB 持久化？](#52-什么是-rdb-持久化)
    - [5.3 RDB 创建快照时会阻塞主线程吗？](#53-rdb-创建快照时会阻塞主线程吗)
    - [5.4 什么是 AOF 持久化？](#54-什么是-aof-持久化)
    - [5.5 AOF 日志是如何实现的？](#55-aof-日志是如何实现的)
    - [5.6 AOF 重写了解吗？](#56-aof-重写了解吗)
    - [5.7 如何选择 RDB 和 AOF？](#57-如何选择-rdb-和-aof)
    - [5.8 Redis 4.0 对于持久化机制做了什么优化？](#58-redis-40-对于持久化机制做了什么优化)
  - [六、Redis 事务](#六redis-事务)
    - [6.1 如何使用 Redis 事务？](#61-如何使用-redis-事务)
    - [6.2 Redis 支持原子性吗？](#62-redis-支持原子性吗)
    - [6.3 如何解决 Redis 事务的缺陷？](#63-如何解决-redis-事务的缺陷)
  - [七、Redis 性能优化](#七redis-性能优化)
    - [7.1 Redis bigkey](#71-redis-bigkey)
      - [什么是 bigkey？](#什么是-bigkey)
      - [bigkey 有什么危害？](#bigkey-有什么危害)
      - [如何发现 bigkey？](#如何发现-bigkey)
    - [7.2 大量 key 集中过期问题](#72-大量-key-集中过期问题)
  - [八、Redis 生产问题](#八redis-生产问题)
    - [8.1 缓存穿透](#81-缓存穿透)
      - [什么是缓存穿透？](#什么是缓存穿透)
    ...
    - [8.2 缓存雪崩](#82-缓存雪崩)
      - [什么是缓存雪崩？](#什么是缓存雪崩)
      - [有哪些解决办法？](#有哪些解决办法-1)
    - [8.3 如何保证缓存和数据库数据的一致性？](#83-如何保证缓存和数据库数据的一致性)
  - [参考](#参考)
